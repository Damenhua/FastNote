...
function CardItem({ note, onDelete, isCalendarPage = false }) {
  ...
  return (
    <div
      className={`relative h-48 w-full min-w-64 max-w-lg rounded-3xl border-2 border-gray-300 bg-transparent p-4 transition-all hover:border-pri-500 ${isDragging ? "shadow-xl" : ""}`}
      ref={setNodeRef}
      style={style}
      {...attributes}
      {...listeners}
